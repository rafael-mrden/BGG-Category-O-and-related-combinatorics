...
# This file was *autogenerated* from the file A5_elementary.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_20 = Integer(20); _sage_const_3 = Integer(3)
...
def check(x,y,s):
    ...
    if len(surviving_summands) >= _sage_const_1  and len(surviving_middle) >= _sage_const_1 :        
        equation = [set(),set()]

        for z in surviving_summands: 
            equation[_sage_const_0 ].add( (z,y) )
        
        for z in surviving_middle:  
            equation[_sage_const_1 ].add( (x,z) )
...
def print_equation(equation):
    string1 = ""
    for p in equation[_sage_const_0 ]:
        string1 += " + M(%s,%s)"%( convert_to_123(p[_sage_const_0 ]) , convert_to_123(p[_sage_const_1 ]) )        

    string2 = ""
    for p in equation[_sage_const_1 ]:
        string2 += " + M(%s,%s)"%( convert_to_123(p[_sage_const_0 ]) , convert_to_123(p[_sage_const_1 ]) )
        
    string3 = string1[_sage_const_3 :] + " == " + string2[_sage_const_3 :]
    return string3+"\n"
...
for eq in elem:
    KReq = [
        set([ (pair[_sage_const_1 ].inverse()*w0, w0*pair[_sage_const_0 ].inverse()) for pair in eq[_sage_const_0 ]]),
        set([ (pair[_sage_const_1 ].inverse()*w0, w0*pair[_sage_const_0 ].inverse()) for pair in eq[_sage_const_1 ]]),
    ]
...
for eq in elem:
    if (len(eq[_sage_const_0 ]) == _sage_const_1  and len(eq[_sage_const_1 ]) == _sage_const_2 ) or (len(eq[_sage_const_0 ]) == _sage_const_2  and len(eq[_sage_const_1 ]) == _sage_const_1 ):
        elem12.append(eq)

elem22 = []    # Equations A+B=C+D

for eq in elem:
    if (len(eq[_sage_const_0 ]) == _sage_const_2  and len(eq[_sage_const_1 ]) == _sage_const_2 ):
        elem22.append(eq)

for eq in elem:
    if (len(eq[_sage_const_0 ]) > _sage_const_2  or len(eq[_sage_const_1 ]) > _sage_const_2 ):
...
for eq in elem:
    if (len(eq[_sage_const_0 ]) == _sage_const_1  and len(eq[_sage_const_1 ]) == _sage_const_1 ):
        
        p = list(eq[_sage_const_0 ])[_sage_const_0 ]
        q = list(eq[_sage_const_1 ])[_sage_const_0 ]
...
for comp in G_components:
    ...
    result2 += "{ " + " , ".join(["%s * %s" %(convert_to_123(p[_sage_const_1 ]),convert_to_123(p[_sage_const_0 ]))   for p in  comp ]) + " }:\n\n"
    
    X = M(comp[_sage_const_0 ][_sage_const_0 ],comp[_sage_const_0 ][_sage_const_1 ])
    X.name = ""
