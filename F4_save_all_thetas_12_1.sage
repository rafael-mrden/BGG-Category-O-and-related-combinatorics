...
this_proc = 1         # for this_proc in range(nr_proc):
...
if this_proc+1 != nr_proc:
    right = (this_proc+1)*size
    W_part = [x for x in W_len[left:right]]
else:
    right = len(W_len)
    W_part = [x for x in W_len[left:]]

print("Running for x in %s"%W_part)
    ...
    for y in W:
        if "(%s, %s)"%(convert_to_123(x),convert_to_123(y.inverse())) in R_order_string:
            if not is_saved( "theta_%s(L(%s))"%(x,y) , "only_bool"):
                save(theta(x,char_L(y)))
                print(CT + ": saved theta_%s(L(%s))"%(x,y))
                print("%d/%d - "%(i,part) + str(datetime.datetime.now()))
    i += 1
    print("*"*20 + "\n%d/%d: x=%s done!\n"%(i,part,convert_to_123(x)) + "*"*20)
